...
metadf <- read.xlsx("Supp Table X - Cell Lines Generated.xlsx", startRow = 3) %>% 
  ...
  mutate(Group = paste0(Treatment, " + ", Titer, " ", Virus))
...
write.csv(final, "combined_RNA_TPM_export.csv", row.names = FALSE)
...
DESeq_df.cleaned$Group <- relevel(DESeq_df.cleaned$Group, ref = "NT + 1X Ad-CMV-Cre")

dds <- DESeq(DESeq_df.cleaned)
resultsNames(dds)
# 
# res <- results(dds, contrast=c("Group", "NT + 1X Ad-CMV-Cre", "Naphthalene (200mg/kg) + 1X HIV-PGK-Cre"))
...
ggsave("RNA-seq cluster heatmap.pdf", plot = cluster_heatmap, height = 4, width = 8)
ggsave("RNA-seq cluster heatmap.png", plot = cluster_heatmap, height = 4, width = 8, dpi = 600)
...
ggsave("RNA-seq PCA.pdf", plot = base_pca, height = 4, width = 8)
ggsave("RNA-seq PCA.png", plot = base_pca, height = 4, width = 8, dpi = 600)
